...
int adrv9025_spi_write(struct no_os_spi_desc *spi, unsigned int reg, unsigned int val)
{
	unsigned char buf[3];
	int ret;
...
//	uint64_t val;
//
//	no_os_mutex_lock(&phy->lock);
//
//	switch ((uint32_t)this_attr->address & 0xFF) {
...
//				phy->cal_mask.calMask &= ~val;
//		} else if (enable) {
//			static const uint32_t INIT_CALS_TIMEOUT_MS =
...
//static int adrv9025_phy_reg_access(struct iio_dev *indio_dev, uint32_t reg,
//				   uint32_t writeval, uint32_t *readval)
//{
//	struct adrv9025_rf_phy *phy = iio_priv(indio_dev);
//	int ret;
//
...
//				 const struct iio_chan_spec *chan, uint32_t mode)
//{
//	struct adrv9025_rf_phy *phy = iio_priv(indio_dev);
...
//		switch (entry->size) {
//		case 1:
//			val = *(uint8_t *)entry->out_value;
//			break;
//		case 2:
//			val = *(u16 *)entry->out_value;
//			break;
//		case 4:
//			val = *(uint32_t *)entry->out_value;
//			break;
//		case 5:
...
//		switch (entry->size) {
//		case 1:
//			*(uint8_t *)entry->out_value = val;
//			break;
//		case 2:
//			*(u16 *)entry->out_value = val;
//			break;
//		case 4:
//			*(uint32_t *)entry->out_value = val;
//			break;
//		case 5:
...
static unsigned long adrv9025_bb_recalc_rate(struct no_os_clk_hw *hw,
...
static int adrv9025_bb_set_rate(struct no_os_clk_hw *hw, unsigned long rate,
				unsigned long parent_rate)
{
//	struct adrv9025_clock *clk_priv = to_clk_priv(hw);
//
...
static long adrv9025_bb_round_rate(struct no_os_clk_hw *hw, unsigned long rate,
				   unsigned long *prate)
{
//	struct adrv9025_clock *clk_priv = to_clk_priv(hw);
...
//		return adrv9025_dev_err(phy);
//
//	pr_warning("GP0 Interrupt Status 0x%llX: %s\n",
		...
		enum jesd204_state_op_reason reason)
{
	...
	ret = no_os_clk_round_rate(phy->dev_clk->clk_desc,
	...
	if (dev_clk > 0 && ((dev_clk / 1000) ==
		...
		no_os_clk_set_rate(phy->dev_clk->clk_desc, dev_clk);
	} else {
...
int adrv9025_init(struct adrv9025_rf_phy **dev, const struct adrv9025_init_param *init_param)
{
	...
	if (ret)
		goto out_spi_init_error;

	ret = no_os_gpio_get_optional(&phy->sysref_req_gpio,
					      init_param->sysref_req_gpio_init);
	if (ret < 0)
		goto out_gpio_init_error;
	if (phy->sysref_req_gpio)
		no_os_gpio_set_value(phy->sysref_req_gpio, 0);

//	clk = devm_clk_get(&spi->dev, "dev_clk");
//	if (IS_ERR(clk))
//		return PTR_ERR(clk);

	phy->spi_device_id = init_param->spi_init->device_id;
//	phy->dev_clk = clk;
//	phy->jdev = jdev;
	no_os_mutex_init(&phy->lock);

	priv = jesd204_dev_priv(jdev);
	priv->phy = phy;

	phy->madDevice = &phy->adi_adrv9025_device;
	phy->no_os_hal.spi = phy->spi_desc;
	phy->no_os_hal.logCfg.logLevel = ADI_HAL_LOG_ERR | ADI_HAL_LOG_WARN;
	phy->madDevice->common.devHalInfo = &phy->no_os_hal;

	strncpy(phy->platformFiles.armImageFile, "ADRV9025_FW.bin;ADRV9025_DPDCORE_FW.bin", sizeof(phy->platformFiles.armImageFile));

	strncpy(phy->platformFiles.streamImageFile, "stream_image_6E3E00EFB74FE7D465FA88A171B81B8F.bin", sizeof(phy->platformFiles.streamImageFile));
...
//	}
	...
	ret = no_os_gpio_get(&phy->no_os_hal.reset_gpio,
						  init_param->reset_gpio_init);
	if (ret)
		goto out_gpio_init_error;
...
//	if (ret) {
//		pr_err("error missing dt property: adi,device-profile-name\n");
//		return -EINVAL;
//	}
//
...
//	if (ret) {
//		pr_err("error missing dt property: adi,init-profile-name\n");
//		return -EINVAL;
//	}
//
...
//	if (ret < 0)
//		pr_warning("%s: failed to register debugfs", __func__);
//
//	if (spi->irq) {
