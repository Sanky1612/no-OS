			     ...
			     uint32_t pageIndex, uint32_t pageSize, uint8_t *rdBuff)
{
	...
	} else if (!strcmp(ImagePath, "stream_image_6E3E00EFB74FE7D465FA88A171B81B8F.bin")) {
		if ((pageIndex * pageSize) > sizeof(stream_image_6E3E00EFB74FE7D465FA88A171B81B8F_bin))
			return -EINVAL;
				      ...
				      uint16_t *phaseOffset, int16_t *digGain)
{
	if (!strcmp(rxGainTablePath, "ADRV9025_RxGainTable.csv")) {
		if (lineCount > sizeof(ADRV9025_RxGainTable) / sizeof(struct ADRV9025_RxGainTableEntry))
			return -EINVAL;
...
{
	if (lineCount > sizeof(ADRV9025_TxAttenTable) / sizeof(struct ADRV9025_TxAttenTableEntry))
		return -EINVAL;
...
FILE* __fopen(const char * filename, const char *mode)
{
	...
	if (!strcmp(filename, "ActiveUseCase.profile")) {
		temp = (char *)no_os_malloc((strlen(json_profile_active_use_case)+1)*sizeof(char));
		...
		temp = (char *)no_os_malloc((strlen(json_profile_active_util_init)+1)*sizeof(char));
		strcpy(temp, json_profile_active_util_init);
		profile.data = temp;
		profile.start = profile.ptr = profile.data;
		profile.end = profile.start + strlen(profile.data);
	} else if (!strcmp(filename, "stream_image_6E3E00EFB74FE7D465FA88A171B81B8F.bin")) {
