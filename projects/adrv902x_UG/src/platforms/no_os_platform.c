	...
	if (halCfg->spiCfg.spiActionDisable == 0) {
		do {
			struct no_os_spi_msg t = {
				.tx_buff = &txData[numTxRxBytes - remaining],
				.rx_buff = &rxData[numTxRxBytes - remaining],
				.bytes_number = (remaining > MAX_SIZE) ? MAX_SIZE :
								remaining,
			};

			result = no_os_spi_transfer(halCfg->spi, &t, 1);
			if (result < 0) {
				halError = ADI_HAL_SPI_FAIL;
			}

			remaining -= t.bytes_number;
		} while (remaining > 0);
	}
...
{
	int32_t ret;
	struct adrv9026_hal_cfg *phal = (struct adrv9002_hal_cfg *)devHalCfg;
...
int32_t no_os_HwReset(void *devHalCfg, uint8_t pinLevel)
{
	...
	no_os_gpio_set_value(halCfg->reset_gpio, pinLevel);
...
int32_t adi_hal_PlatformSetup(void *devHalInfo, adi_hal_Platforms_e platform)
{
    ...
    switch (platform)
    {
    case ADI_NO_OS_PLATFORM:
    	adi_hal_HwOpen 			= no_os_HwOpen;
		adi_hal_HwClose         = no_os_HwClose;
		adi_hal_HwReset         = no_os_HwReset;
//		adi_hal_DevHalCfgCreate = no_os_DevHalCfgCreate;
//		adi_hal_DevHalCfgFree   = no_os_DevHalCfgFree;
		adi_hal_HwVerify 		= no_os_HwOpen;

		adi_hal_SpiInit              = no_os_SpiInit; /* TODO: remove?  called by HwOpen() */
		adrv9025_hal_SpiWrite             = no_os_SpiWrite;
		adrv9025_hal_SpiRead              = no_os_SpiRead;
...
//		adi_hal_BbicRegisterRead   = no_os_BbicRegisterRead;
//		adi_hal_BbicRegisterWrite  = no_os_BbicRegisterWrite;
//		adi_hal_BbicRegistersRead  = no_os_BbicRegistersRead;
//		adi_hal_BbicRegistersWrite = no_os_BbicRegistersWrite;
